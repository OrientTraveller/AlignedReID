...
from TriHard_Loss import TripletLoss#！！与表征学习不同！！引入难样本挖掘三元组损失
from sampler import RandomIdentitySampler#！！与表征学习不同！！引入采样器
import transform as T

"""
本文件是行人重识别系统的核心文件，用于度量学习模型训练。
"""
#设定输入参数
width=64                    #图片宽度
height=128                 #图片高度
...
evaluate=False           #是否测试
margin=0.3                 #！！与表征学习不同！！TripletHard Loss计算的margin参数
num_instances=4        #！！与表征学习不同！！每个ID图片数，一定要能被batch_size整除
max_acc=-1#最大准确率
...
def main():
    ...
    train_data_loader=DataLoader(
        ...
        sampler=RandomIdentitySampler(dataset.train, num_instances=num_instances),#！！与表征学习不同！！度量学习需要在声明训练数据吞吐器时指定采样器
    ...
    #加载模型
    model=ReIDNet(num_classes=751,loss={'softmax, metric'})#指定分类的数量，与使用的损失函数以便决定模型输出何种计算结果
    ...
    criterion_metric=TripletLoss(margin=margin)#！！与表征学习不同！！还需要指定一个度量损失函数
...
def train(epoch, model, criterion_class, criterion_metric, optimizer, scheduler, data_loader):
    ...
    for batch_idx, (imgs, pids, cids) in enumerate(data_loader):
        ...
        xent_loss=criterion_class(outputs,pids)#！！与表征学习不同！！计算分类损失
        metric_loss=criterion_metric(features, pids)#！！与表征学习不同！！计算度量损失
        loss=xent_loss+metric_loss#！！与表征学习不同！！计算总损失
